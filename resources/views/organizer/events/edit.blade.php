    ...
    <div class="min-h-screen bg-gray-50 py-8">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <a href="{{ route('organizer.events.attendees.contact', $event) }}"
                           class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 012 2z"></path>
                            </svg>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    <form method="POST" action="{{ route('organizer.events.update', $event) }}" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <select id="event_type" name="event_type" required
                                        ...
                                        <option value="external" {{ old('event_type', $event->event_type) == 'external' ? 'selected' : '' }}>Externe Veranstaltung</option>
                                    </select>
                                    @error('event_type')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                        ...
                        <div class="bg-white rounded-lg shadow-md p-6">
                            ...
                            <div class="mt-4">
                                <!-- Hidden field removed - we use AJAX now -->
                                <label class="flex items-start cursor-pointer">
                                    <input type="checkbox" id="has_multiple_dates"
                                           {{ $event->has_multiple_dates ? 'checked' : '' }}
                                ...
                                <div id="multiple-dates-info" class="mt-3 p-4 bg-blue-50 border-l-4 border-blue-500 rounded" style="display: {{ old('has_multiple_dates', $event->has_multiple_dates) ? 'block' : 'none' }}">
                                    <div class="flex items-start">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-xs text-blue-700 mt-1">
                                                @if($event->has_multiple_dates && $event->dates->count() > 0)
                                                    Dieses Event hat {{ $event->dates->count() }} Termine. Scrollen Sie nach unten zur Terminverwaltung, um weitere Termine hinzuzufügen oder bestehende zu verwalten.
                                                @elseif($event->has_multiple_dates)
                                                    <strong>Speichern Sie das Event</strong>, um auf die Terminverwaltung zuzugreifen und weitere Termine hinzuzufügen.
                                                    Das oben angegebene Start- und Enddatum wird als erster Termin gespeichert.
                                                @else
                                                    <strong>Wichtig:</strong> Speichern Sie das Event nach dem Aktivieren dieser Option.
                                                    Danach erscheint hier eine Terminverwaltung, mit der Sie weitere Termine hinzufügen können.
                                                @endif
                                            </p>
                                            ...
                                            <div class="mt-3 flex items-center gap-2 text-xs bg-white rounded p-2 border border-blue-200">
                                                ...
                                                <span class="text-blue-800">
                                                    <strong>Nächster Schritt:</strong> Klicken Sie unten auf "Änderungen speichern", um die Terminverwaltung freizuschalten.
                                                </span>
                                            </div>
                                            @endif
                                        </div>
                        ...
                        <!-- Zusatzinformationen -->
                        <div id="pricing-section" class="bg-white rounded-lg shadow-md p-6">
                        ...
                        <div id="external-section" class="bg-white rounded-lg shadow-md p-6" style="display: {{ old('event_type', $event->event_type) === 'external' ? 'block' : 'none' }};">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="url" id="external_booking_url" name="external_booking_url"
                                ...
                                <div>
                                    ...
                                    <input type="text" id="external_booking_button_text" name="external_booking_button_text"
                                           value="{{ old('external_booking_button_text', $event->external_booking_button_text) }}"
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <textarea id="ticket_notes" name="ticket_notes" rows="4"
                                              ...
                                              class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('ticket_notes', $event->ticket_notes) }}</textarea>
                                ...
                                <div>
                                    <label class="flex items-center">
                                        ...
                                        <input type="checkbox" name="requires_ticket" value="1"
                                               {{ old('requires_ticket', $event->requires_ticket ?? true) ? 'checked' : '' }}
                                               id="requires_ticket"
                                ...
                                <div id="qr_code_option">
                                    <label class="flex items-center">
                                        ...
                                        <input type="checkbox" name="show_qr_code_on_ticket" value="1"
                                               {{ old('show_qr_code_on_ticket', $event->show_qr_code_on_ticket ?? true) ? 'checked' : '' }}
                                               class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                ...
                <!-- Rechte Spalte: Ticket-Typen und Termine-Verwaltung -->
                <div class="space-y-6">
                    <!-- Ticket-Typen Block (bereits außerhalb des Formulars) -->
                    <div id="ticket-types-section" class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <!-- Multiple Dates Partial (verschoben in rechte Spalte) -->
                    {{-- Include Multiple Dates Management (außerhalb des Haupt-Event-Formulars) --}}
                    @include('organizer.events.partials.multiple-dates')
                </div>
            </div>

            <!-- Skripte (bleiben am Ende der Seite) -->
            <script>
                ...
                // Toggle Multiple Dates Info Box via AJAX
                ...
                if (multipleDatesCheckbox && multipleDatesInfo) {
                    // Initial state
                    multipleDatesInfo.style.display = multipleDatesCheckbox.checked ? 'block' : 'none';

                    multipleDatesCheckbox.addEventListener('change', async function() {
                        const isChecked = this.checked;
                        const eventId = this.dataset.eventId;

                        // Bestätigungsdialog bei Deaktivierung mit existierenden Terminen
                        @if($event->has_multiple_dates && $event->dates->count() > 0)
                        if (!isChecked) {
                            if (!confirm('Achtung: Wenn Sie diese Option deaktivieren, werden ALLE {{ $event->dates->count() }} Termine gelöscht. Fortfahren?')) {
                                this.checked = true;
                                return;
                            }
                        }
                        ...
                        // Disable checkbox während der Anfrage
                        this.disabled = true;

                        try {
                            const response = await fetch(`/organizer/events/${eventId}/multiple-dates/toggle`, {
                                method: 'POST',
                                headers: {
                            ...
                            if (data.success) {
                                ...
                                // Seite neu laden wenn angefordert
                                if (data.reload) {
                                    setTimeout(() => {
                                    ...
                                    multipleDatesInfo.style.display = isChecked ? 'block' : 'none';
                                    if (isChecked) {
                            ...
                            alertDiv.textContent = 'Fehler: ' + error.message;
                            document.body.appendChild(alertDiv);
                ...
                function updateSections() {
                    ...
                    // Hide pricing, ticket settings and ticket types for external events
                    if (pricingSection) pricingSection.style.display = isExternal ? 'none' : '';
                    if (ticketSettingsSection) ticketSettingsSection.style.display = isExternal ? 'none' : '';
                    if (ticketTypesSection) ticketTypesSection.style.display = isExternal ? 'none' : '';
                }
